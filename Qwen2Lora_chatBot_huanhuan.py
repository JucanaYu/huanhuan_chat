...
import torch    #用于模型的加载和计算
import streamlit as st

# 在侧边栏中创建一个标题和一个链接
with st.sidebar:
    st.markdown("## 这是来自一个优秀的chat开源项目")
...
# 定义模型路径
mode_name_or_path = 'C:/Users/17101/Desktop/huanhuan-chat-qwen2-7B/huanhuan-Qwen2-chat'

# 定义一个函数，用于获取模型和tokenizer
# 使用装饰器@st.cache_resource缓存资源函数get_model()，以避免每次调用时都重新加载模型和tokenize
@st.cache_resource
def get_model():
    # 从预训练的模型中获取tokenizer
